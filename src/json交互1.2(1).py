"""
缺点：主窗口的任意项在每次编辑后完成后不可以立即再次编辑该项，需要重启项目   (已解决)
"""
...
def make_values():
    ...
    if len(data["count_tasks"]) != 0:
        for i in range(0, len(data["count_tasks"])):
            id, name = (
                data["count_tasks"][i]["id"],
                data["count_tasks"][i]["name"],
            )
            combobox_count_tasks_values.append(f"id:{id} {name}")
    if len(data["timer_tasks"]) != 0:
        for i in range(0, len(data["timer_tasks"])):
            id, name = (
                data["timer_tasks"][i]["id"],
                data["timer_tasks"][i]["name"],
            )
            combobox_timer_tasks_values.append(f"id:{id} {name}")
...
def edit_lockers():
    ...
    if lockers_state.get() == "新建":
        ...
        for i in range(0, len(data["lockers"])):
            if (
                data["lockers"][i]["name"] == lockers_state.get()
            ):  ###特别注意：不同lockers之间不能重名，sure_locker前需要检查并且错误弹窗
                index = i
        ...
        get_lockers_name = tk.StringVar(
            value=data["lockers"][index]["name"]
        )
        get_lockers_punish = tk.StringVar(
            value=data["lockers"][index]["punish"]
        )
        get_lockers_type = tk.StringVar(
            value=data["lockers"][index]["list_type"]
        )
    ...
    def edit_list():
        ...
        def add_list(i=""):
            ...
            new_button_sure = tk.Button(
                f,
                text="确认",
                command=lambda: sure_this_list(new_entry_text),
            )
            new_button = tk.Button(
                f,
                text="删除",
                command=lambda: remove_entry_and_button(
                    new_entry,
                    new_button,
                    new_button_sure,
                    new_entry_text,
        ...
        def remove_entry_and_button(
            entry, button, button_sure, new_entry_text
        ):
            nonlocal counter
            counter -= 1
        ...
        button_add = tk.Button(
            window_lockers_list, text="新增", command=add_list
        )
        button_add.place(x=0, y=0)
        button_sure_lockers_list = tk.Button(
            window_lockers_list,
            text="确定",
            command=sure_lockers_list,
        )
    ...
    def edit_time_rulers():
        ...
        window_lockers_time_rules.title("lockers_设置时间(最多三条)")
        ...
        def add_rules(i=None):
            ...
            combobox_start_time = ttk.Combobox(
                frame,
                values=["示范： 06：00（24小时计时法）"],
                textvariable=start_time,
            )
            label_end_time = tk.Label(frame, text="结束时间")
            combobox_end_time = ttk.Combobox(
                frame, values=["示范： 23：30"], textvariable=end_time
            )
            label_days = tk.Label(frame, text="日期")
            combobox_days = ttk.Combobox(
                frame,
                values=[
                    "说明：",
                    "请输入一个在1到7之间的数",
            ...
            button_sure_this_rule = tk.Button(
                frame,
                text="确认",
                command=lambda: sure_this_rule(
                    start_time, end_time, days
                ),
            )
            button_delete_this_rule = tk.Button(
                frame,
                text="删除",
                command=lambda: remove_object(
                    label_start_time,
                    label_end_time,
                    label_days,
                    combobox_start_time,
                    combobox_end_time,
                    combobox_days,
                    button_sure_this_rule,
                    button_delete_this_rule,
                    start_time,
                    end_time,
                    days,
                ),
        ...
        def remove_object(
            ...
            counter -= 1
            if {
                "start_time": f"{start.get()}",
                "end_time": f"{end.get()}",
                "days": [f"{d.get()}"],
            } in get_time_rules_m:
                get_time_rules_m.remove(
                    {
                        "start_time": f"{start.get()}",
                        "end_time": f"{end.get()}",
                        "days": [f"{d.get()}"],
        ...
        def sure_time_rules():
            for i in get_time_rules_m:
                if i not in get_time_rules and i != {
                    "start_time": "",
                    "end_time": "",
                    "days": [""],
                }:
        ...
        button_add = tk.Button(
            window_lockers_time_rules, text="新增", command=add_rules
        )
        button_add.place(x=0, y=0)
        button_sure_time_rules = tk.Button(
            window_lockers_time_rules,
            text="完成",
            command=sure_time_rules,
        )
    ...
    def sure_lockers():
        ...
        if lockers_state.get() == "新建":
            ...
            for i in data["lockers"]:
                if (
                    i["name"] == get_lockers_name.get()
                    and i != data["lockers"][index]
                ):
                    rename = True
        if rename:
        ...
        if lockers_state.get() == "新建":
            ...
            combobox_lockers_values.append("新建")
            combobox_lockers.config(
                values=combobox_lockers_values
            )  ##更新下拉列表
        else:
            combobox_lockers_values.remove("新建")
            combobox_lockers_values.remove(
                data["lockers"][index]["name"]
            )
            data["lockers"][index] = locker_data
            combobox_lockers_values.append(
                data["lockers"][index]["name"]
            )
            combobox_lockers_values.append("新建")
    ...
    entry_name = tk.Entry(
        window_lockers, textvariable=get_lockers_name, width=50
    )
    button_ifon = tk.Button(
        window_lockers,
        width=50,
        command=ifon,
        textvariable=get_lockers_ifon,
    )
    combobox_punish = ttk.Combobox(
        window_lockers,
        textvariable=get_lockers_punish,
        width=50,
        values=["MINIMIZE", "CLOSE", "DEBUG", "LAG"],
    )
    combobox_type = ttk.Combobox(
        window_lockers,
        values=["BLACKLIST", "WHITELIST"],
        textvariable=get_lockers_type,
        width=47,
    )
    button_list = tk.Button(
        window_lockers, text="管理名单", width=15, command=edit_list
    )
    button_time_rules = tk.Button(
        window_lockers,
        text="时间设置",
        width=15,
        command=edit_time_rulers,
    )
    button_sure_lockers = tk.Button(
        window_lockers, text="确定", width=15, command=sure_lockers
    )
    button_delete_lockers = tk.Button(
        window_lockers, text="删除", width=15, command=delete_lockers
    )
...
def edit_count_tasks():
    ...
    if count_tasks_state.get() == "新建":
        ...
        if count_tasks["deadline_aim"] == None:
            ...
            if "aim" in count_tasks["deadline_aim"]:
                get_deadline_aim.set(
                    count_tasks["deadline_aim"]["aim"]
                )
    ...
    label_activate = tk.Label(
        window_count_tasks, text="启用 ", width=15
    )
    label_daily_aim = tk.Label(
        window_count_tasks, text="每日目标 ", width=15
    )
    label_weekly_aim = tk.Label(
        window_count_tasks, text="每周目标 ", width=15
    )
    label_monthly_aim = tk.Label(
        window_count_tasks, text="每月目标 ", width=15
    )
    label_deadline = tk.Label(
        window_count_tasks, text="截止日期 ", width=15
    )
    label_deadline_aim = tk.Label(
        window_count_tasks, text="截止前目标 ", width=15
    )
    ...
    def sure_count_tasks():
        ...
        if count_tasks_state.get() == "新建":
            ...
            combobox_count_tasks_values.append(f"id:{id} {name}")
            combobox_count_tasks_values.append("新建")
            combobox_count_tasks.config(
                values=combobox_count_tasks_values
            )  ###修改主窗口的下拉
        else:
            id, name = (
                data["count_tasks"][index]["id"],
                data["count_tasks"][index]["name"],
            )
            combobox_count_tasks_values.remove(f"id:{id} {name}")
            combobox_count_tasks_values.remove("新建")
    ...
    ##其余组件的创建与布局
    entry_id = tk.Entry(
        window_count_tasks, textvariable=get_id, width=20
    )
    entry_name = tk.Entry(
        window_count_tasks, textvariable=get_tasks_name, width=20
    )
    entry_note = tk.Entry(
        window_count_tasks, textvariable=get_note, width=20
    )
    button_ifon = tk.Button(
        window_count_tasks,
        textvariable=get_ifon,
        width=20,
        command=click_ifon,
    )
    combobox_daily_aim = ttk.Combobox(
        window_count_tasks,
        values=["null", "请输入次数"],
        textvariable=get_daily_aim,
        width=17,
    )
    combobox_weekly_aim = ttk.Combobox(
        window_count_tasks,
        values=["null", "请输入次数"],
        textvariable=get_weekly_aim,
        width=17,
    )
    combobox_monthly_aim = ttk.Combobox(
        window_count_tasks,
        values=["null", "请输入次数"],
        textvariable=get_monthly_aim,
        width=17,
    )
    combobox_deadline_aim = ttk.Combobox(
        window_count_tasks,
        values=["null", "请输入次数"],
        textvariable=get_deadline_aim,
        width=17,
    )
    combobox_deadline = ttk.Combobox(
        window_count_tasks,
        values=["null", "请输入日期", "示范：2024-12-31"],
        width=17,
        textvariable=get_deadline,
    )
    button_sure = tk.Button(
        window_count_tasks,
        text="确认",
        width=10,
        command=sure_count_tasks,
    )
    button_delete = tk.Button(
        window_count_tasks,
        text="删除",
        width=10,
        command=delete_sure_tasks,
    )
...
def edit_timer_tasks():
    ...
    if timer_tasks_state.get() == "新建":
        ...
        if data["timer_tasks"][index]["monthly_aim"] == None:
            ...
            monthly_aim = str(
                data["timer_tasks"][index]["monthly_aim"]
            )
        if data["timer_tasks"][index]["deadline_aim"] == None:
            ...
            if (
                "data" in data["timer_tasks"][index]["deadline_aim"]
            ) and (
                "aim" in data["timer_tasks"][index]["deadline_aim"]
            ):
                date = data["timer_tasks"][index]["deadline_aim"][
                    "date"
                ]
                aim = data["timer_tasks"][index]["deadline_aim"][
                    "aim"
                ]
        ...
        get_ifon.set(
            ifon_dict[data["timer_tasks"][index]["activate"]]
        )
    ...
    def sure():
        ...
        if timer_tasks_state.get() == "新建":
            ...
            combobox_timer_tasks_values.append(f"id:{id} {nm}")
            combobox_timer_tasks_values.append("新建")
            combobox_timer_tasks.config(
                values=combobox_timer_tasks_values
            )  ###修改主窗口的下拉
        else:
            id, nm = (
                data["timer_tasks"][index]["id"],
                data["timer_tasks"][index]["name"],
            )
            combobox_timer_tasks_values.remove(f"id:{id} {nm}")
            combobox_timer_tasks_values.remove("新建")
    ...
    def edit_timer():
        ...
        def add_rules(i=None):
            ...
            combobox_timer_rules = ttk.Combobox(
                frame,
                values=[
                    "请输入分钟数",
                    "或者预设的时间集",
                ...
                textvariable=get_timer_ruler,
            )
            button_delete_this_rule = tk.Button(
                frame,
                text="删除",
                command=lambda: delete_this_rule(
                    get_timer_ruler.get(),
                    label_timer_ruler,
                    combobox_timer_rules,
                    button_sure_this_rule,
                    button_delete_this_rule,
                ),
            )
            button_sure_this_rule = tk.Button(
                frame,
                text="确认",
                command=lambda: sure_this_rule(get_timer_ruler.get()),
            )

            label_timer_ruler.pack(
                side="top", fill="x", padx=10, pady=5
            )
            combobox_timer_rules.pack(
                side="top", fill="x", padx=10, pady=5
            )
            button_sure_this_rule.pack(
                side="top", fill="x", padx=10, pady=5
        ...
        for i in get_timer_list:
            add_rules(i)
        button_sure_all_rules = tk.Button(
            window_edit_timer, text="完成", command=sure_all_rule
        )
        button_add_timer = tk.Button(
            window_edit_timer, text="新增", command=add_rules
        )
    ...
    label_timers = tk.Label(
        window_timer_tasks, text="可选计时", width=15
    )
    label_daily_aim = tk.Label(
        window_timer_tasks, text="每日目标", width=15
    )
    label_weekly_aim = tk.Label(
        window_timer_tasks, text="每周目标", width=15
    )
    label_monthly_aim = tk.Label(
        window_timer_tasks, text="每月目标", width=15
    )
    label_aim = tk.Label(
        window_timer_tasks, text="截止日期前目标", width=15
    )
    label_deadline = tk.Label(
        window_timer_tasks, text="截止日期", width=15
    )
    ...
    # 其余组件的创建与布局
    entry_id = tk.Entry(
        window_timer_tasks, textvariable=get_id, width=20
    )
    entry_name = tk.Entry(
        window_timer_tasks, textvariable=get_timer_name, width=20
    )
    entry_note = tk.Entry(
        window_timer_tasks, textvariable=get_note, width=20
    )
    button_ifon = tk.Button(
        window_timer_tasks,
        textvariable=get_ifon,
    ...
    button_timers = tk.Button(
        window_timer_tasks, text="编辑", width=20, command=edit_timer
    )
    combobox_daily_aim = ttk.Combobox(
        window_timer_tasks,
        values=["null", "请输入目标时长"],
    ...
    combobox_aim = ttk.Combobox(
        window_timer_tasks,
        values=["null", "请输入目标时长"],
        textvariable=get_aim,
        width=17,
    )
    combobox_deadline = ttk.Combobox(
        window_timer_tasks,
        values=["null", "请输入请输入日期,", "示范：2024-12-31"],
        textvariable=get_date,
        width=17,
    )
    button_sure = tk.Button(
        window_timer_tasks, text="确认", width=10, command=sure
    )
    button_delete = tk.Button(
        window_timer_tasks, text="删除", width=10, command=delete
    )
...
def edit_preset():
    ...
    get_timer = tk.StringVar(
        value=list_to_str(data["presets"]["TIMERS"])
    )
    get_days = {}
    ...
    def edit_black():
        ...
        def add(i=None):
            ...
            button_sure_this = tk.Button(
                f,
                text="确认",
                width=20,
                height=1,
                command=lambda: sure_this(black.get()),
            )
            button_delete = tk.Button(
                f,
                text="删除",
        ...
        for i in get_black:
            add(i)
        button_add = tk.Button(
            window_black, text="新增", width=10, command=add
        )
        button_sure_all_black = tk.Button(
            window_black,
            text="完成",
            width=10,
            command=sure_all_black,
        )
    ...
    def edit_white():
        ...
        def add(i=None):
            ...
            button_sure_this = tk.Button(
                ...
                width=20,
                height=1,
                command=lambda: sure_this(white.get()),
            )
            button_delete = tk.Button(
                f,
                text="删除",
        ...
        for i in get_white:
            add(i)
        button_add = tk.Button(
            window_white, text="新增", width=10, command=add
        )
        button_sure_all_white = tk.Button(
            window_white,
            text="完成",
            width=10,
            command=sure_all_white,
        )
    ...
    def edit_days():
        ...
        def add(i=None):
            ...
            for ii in i:
                ...
                entry_name = tk.Entry(
                    frame1, width=20, textvariable=days_name
                )
                entry_value = tk.Entry(
                    frame1, width=20, textvariable=days_value
                )
                button_sure_this_day = tk.Button(
                    frame1,
                    text="确定",
                    width=20,
                    command=lambda: sure_this_day(
                        days_name.get(), days_value.get()
                    ),
                )
                button_delete = tk.Button(
                    frame1,
        ...
        button_add = tk.Button(
            window_days, text="新增", width=10, command=add
        )
        button_sure_all_days = tk.Button(
            window_days, text="完成", width=10, command=sure_all_days
        )
        button_add.place(x=0, y=0)
    ...
    button_black = tk.Button(
        window_preset, text="编辑", width=40, command=edit_black
    )
    button_white = tk.Button(
        window_preset, text="编辑", width=40, command=edit_white
    )
    entry_timers = tk.Entry(
        window_preset, width=40, textvariable=get_timer
    )
    button_days = tk.Button(
        window_preset, text="编辑", width=40, command=edit_days
    )
    button_sure = tk.Button(
        window_preset, text="确定", width=20, command=sure
    )
...
combobox_lockers = ttk.Combobox(
    window, textvariable=lockers_state, values=combobox_lockers_values
)
combobox_count_tasks = ttk.Combobox(
    window,
    textvariable=count_tasks_state,
    values=combobox_count_tasks_values,
)
combobox_timer_tasks = ttk.Combobox(
    window,
    textvariable=timer_tasks_state,
    values=combobox_timer_tasks_values,
)
...
button_count_tasks = tk.Button(
    window, text="编辑", command=edit_count_tasks
)
button_timer_tasks = tk.Button(
    window, text="编辑", command=edit_timer_tasks
)
