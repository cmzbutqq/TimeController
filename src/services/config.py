...
import json
from .utils.helper import Union, singleton, dataclass, Callable
...
class Config:
    ...
    # def unsafe_get(self, *keys) -> jval_t:
    #     """
    #     通过keys找到值 这个值是引用 更改会同步到self.cfg里面
    #     注意：如果你把返回值复制给了一个变量并对它改来改去
    #     (t=self.get('key1','key2); t['key3']=...)
    #     t和cfg是同步改变的, 我们也默认二者同步
    #     但如果你调用了load(), t和cfg会失去同步(cfg指向了新的对象)
    #     因此, save()随便用, load()慎用
    #     """
    #     cur = self.cfg
    #     for key in keys:
    ...
    def keys(self, *keys) -> tuple:  # 通过keys找到值 返回这个值的所有key
        cur = self.get(*keys)()
