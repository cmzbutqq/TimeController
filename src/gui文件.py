...
class ModernApp:
    def __init__(self, root):
        ...
        self.file_watcher = FileWatcher(
            user_json_path.user_json_path, self.on_file_change
        )
        # 在后台线程中运行文件监视器
        threading.Thread(
            target=self.file_watcher.watch, daemon=True
        ).start()

        # 配置样式
        self.style = ttk.Style()
        self.style.configure(
            "Sidebar.TFrame", background="#252526"
        )  # 按钮栏深灰色
        self.style.configure(
            "Content.TFrame", background="#1e1e1e"
        )  # 内容区深灰色
        self.style.configure(
            "AppTitle.TLabel",
            font=("Arial", 18, "bold"),
        ...
        self.sidebar = ttk.Frame(
            self.main_frame, style="Sidebar.TFrame", width=200
        )
        self.sidebar.pack(side=tk.LEFT, fill=tk.Y)

        # 右侧动态内容区
        self.content = ttk.Frame(
            self.main_frame, style="Content.TFrame"
        )
    ...
    def on_file_change(self):
        ...
        deleted_timer_tasks = [
            task
            for task in old_timer_tasks
            if task not in new_timer_tasks
        ]

        # 对于每个被删除的timer_task，删除其记录
        for task in deleted_timer_tasks:
            excel_file_path = "data/timer_tasks_record.xlsx"
            TaskRecorder.delete_task_records_from_excel(
                task["id"], excel_file_path
            )

        # 查找并处理被删除的 count_tasks
        deleted_count_tasks = [
            task
            for task in old_count_tasks
            if task not in new_count_tasks
        ]
    ...
    def show_locker(self):
        ...
        for locker in locker_manager.lockers:
            ...
            status_color = (
                "#4caf50" if locker.on else "#f44336"
            )  # 绿色/红色
    ...
    def show_timer_tasks(self):
        ...
        # 创建下拉菜单来选择计时器任务
        self.timer_tasks_combobox = ttk.Combobox(
            self.content, values=self.get_timer_task_names()
        )
        ...
        load_task_button = ttk.Button(
            self.content,
            text="Load Task",
            command=self.load_selected_timer_task,
        )
    ...
    def load_selected_timer_task(self):
        ...
        if timer_task:
            if messagebox.askokcancel(
                "Load Task",
                f"Do you want to load the task '{task_name}'?",
            ):
    ...
    def show_timer_task_info(self, timer_task):
        ...
        task_id_label = ttk.Label(
            self.control_frame, text=f"ID: {task_id}"
        )
        task_id_label.grid(row=0, column=0, padx=10, pady=10)
        task_name_label = ttk.Label(
            self.control_frame, text=f"Name: {task_name}"
        )
        task_name_label.grid(row=1, column=0, padx=10, pady=10)
        task_note_label = ttk.Label(
            self.control_frame, text=f"Note: {task_note}"
        )
        task_note_label.grid(row=2, column=0, padx=10, pady=10)

        # 显示任务目标
        daily_aim_label = ttk.Label(
            self.control_frame,
            text=f"Daily Aim: {timer_task['daily_aim']}",
        )
        daily_aim_label.grid(row=3, column=0, padx=10, pady=10)
        weekly_aim_label = ttk.Label(
            self.control_frame,
            text=f"Weekly Aim: {timer_task['weekly_aim']}",
        )
        weekly_aim_label.grid(row=4, column=0, padx=10, pady=10)
        monthly_aim_label = ttk.Label(
            self.control_frame,
            text=f"Monthly Aim: {timer_task['monthly_aim']}",
        )
        monthly_aim_label.grid(row=5, column=0, padx=10, pady=10)

        # 获取并显示已记录时间
        total_recorded_time = (
            self.timer_tasks_manager.get_total_recorded_time(task_id)
        )
        total_time_label = ttk.Label(
            self.control_frame,
            text=f"Total Recorded Time: {total_recorded_time}",
        )
        ...
        start_button = ttk.Button(
            self.control_frame,
            text="Start",
            command=lambda: self.timer_tasks_manager.start_timer(
                task_id
            ),
        )
        ...
        pause_button = ttk.Button(
            self.control_frame,
            text="Pause",
            command=lambda: self.timer_tasks_manager.pause_timer(
                task_id
            ),
        )
        ...
        elapsed_time_label = ttk.Label(
            self.control_frame, textvariable=self.elapsed_time_var
        )
        ...
        def update_elapsed_time():
            ...
            if timer and timer.running:
                ...
                # 计时器已停止或暂停，显示已记录的总时间
                self.elapsed_time_var.set(
                    f"Total Recorded Time: {total_recorded_time}"
                )
    ...
    def stop_timer_with_record(self, task_id):
        """停止计时器任务并保存记录"""
        self.timer_tasks_manager.stop_timer(
            task_id, note="Task completed manually"
        )
    ...
    def update_timer_label(self, task_id):
        """更新计时标签以显示经过的时间"""
        if (
            task_id in self.timer_tasks_manager.timers
            and self.timer_tasks_manager.timers[task_id].running
        ):
            elapsed_time = (
                datetime.now()
                - self.timer_tasks_manager.timers[task_id].start
            )
            self.timer_label.config(
                text=f"Time: {str(elapsed_time).split('.')[0]}"
            )  # 显示小时、分钟和秒
            # 每秒更新一次
            self.root.after(
                1000, lambda: self.update_timer_label(task_id)
            )
        else:
            # 计时器已停止，显示总有效时间
            total_time = (
                self.timer_tasks_manager.get_total_recorded_time(
                    task_id
                )
            )
            self.timer_label.config(
                text=f"Total Time: {str(total_time).split('.')[0]}"
            )
    ...
    def show_count_tasks(self):
        ...
        for task in self.count_tasks_manager.tasks:
            ...
            task_button = ttk.Button(
                self.content,
                text=btn_text,
                command=lambda task_id=task[
                    "id"
                ]: self.on_task_button_click(task_id),
                width=50,  # 设置按钮宽度
