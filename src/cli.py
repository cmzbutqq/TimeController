...
def make_layout(title: str, instructs: str) -> Layout:
    ...
    footer = Panel(
        Text(instructs, justify="center"), border_style="cyan"
    )
...
def cfg_tasks() -> Table:
    ...
    for task in config.get("count_tasks")():
        ...
        ddl = (
            f"{ddl['aim']} times until: {ddl['date']}"
            if ddl is not None
            else "None"
        )
        table.add_row(
...
def add_locker():
    ...
    list_type = (
        "WHITELIST" if list_type.lower() == "w" else "BLACKLIST"
    )
    ...
    while True:
        start_time = input(
            "start time (HH:MM:SS) (input q to stop): "
        )
...
def add_task():
    ...
    if tp == "timer":
        timers = input(
            "timer presets(in minutes,0 means countup,seperate by comma): "
        )
        timers = timers.split(",")
    ...
    weekly_aim = (
        None if weekly_aim.lower() == "n" else int(weekly_aim)
    )

    monthly_aim = input(f"monthly {unit} aim (n for no aim): ")
    monthly_aim = (
        None if monthly_aim.lower() == "n" else int(monthly_aim)
    )

    deadline_aim = input(
        f"deadline {unit} aim (n for no aim)(fmt: YYYY-MM-DD , your aim): "
    )
    if deadline_aim.lower() == "n":
        ...
        deadline_aim = {"date": date, "aim": aim}
...
def start_timer():
    ...
    for task in config.get("count_tasks")():
        if task["id"] == id:
            note = input("any note? : ")
            TaskRecorder.add_record(
                TaskRecorder.counter_record(id, datetime.now(), note)
            )
            name = task["name"]
...
def track_timers():
    ...
    layout = make_layout(
        "RUNNING TIMERS",
        "q: main menu\tp:pause all\te:stop all\tr:resume all\t",
    )
        ...
        while keybd_command not in ["q"]:
            ...
            for timer in TaskTimer.instances:
                ...
                if status.countdown is None:
                    ...
                    running = Spinner("dots", speed=speed)
                    percentage = round(
                        status.used_time / status.countdown * 100, 1
                    )
                    grid.add_row(
                        Text(name + " "),
                        running,
